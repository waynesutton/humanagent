...
// Primary nav: the core daily workflow items
const primaryNavItems: Array<NavItem> = [
  ...
  },
  {
    ...
    icon: (
      <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
        <path strokeLinecap="round" strokeLinejoin="round" d="M17 20h5v-1a4 4 0 00-5-3.87M17 20H7m10 0v-1c0-.653-.126-1.277-.356-1.848M7 20H2v-1a4 4 0 015-3.87M7 20v-1c0-.653.126-1.277.356-1.848m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2.5 2.5 0 11-5 0 2.5 2.5 0 015 0zM8 10a2.5 2.5 0 11-5 0 2.5 2.5 0 015 0z" />
      </svg>
    ),
  },
  {
  ...
  },
  {
    ...
    icon: (
      <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
        <path strokeLinecap="round" strokeLinejoin="round" d="M7.5 8.25h9m-9 3h6m-8.25 8.25h12.75A2.25 2.25 0 0020.25 17.25V6.75A2.25 2.25 0 0018 4.5H6A2.25 2.25 0 003.75 6.75v10.5A2.25 2.25 0 006 19.5z" />
      </svg>
    ),
  },
  {
...
// Secondary nav: accessible via "More" dropdown
...
export function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const isSecondaryActive = secondaryNavItems.some((item) => location.pathname === item.href);

  // Mobile: show first 4 primary items in bottom bar, rest go in "More"
  const mobileBarItems = primaryNavItems.slice(0, 4);
  const mobileMoreItems = [...primaryNavItems.slice(4), ...secondaryNavItems];
  const mobileBarHrefs = new Set(mobileBarItems.map((item) => item.href));
  const isMobileMoreActive = [...primaryNavItems, ...secondaryNavItems].some(
    (item) => location.pathname === item.href && !mobileBarHrefs.has(item.href)
  );
    ...
    <div className="min-h-screen bg-surface-0">
          ...
          <div className="flex items-center gap-6">
            ...
            <nav className="hidden items-center gap-1 lg:flex">
              ...
              <div className="relative" ref={moreRef}>
                ...
                {moreOpen ? (
                  <div className="absolute left-0 z-50 mt-2 w-52 rounded-lg border border-surface-3 bg-surface-0 p-1 shadow-card">
                    {secondaryNavItems.map((item) => (
                      <Link
                        ...
                        className={`flex w-full items-center gap-2.5 rounded-md px-3 py-2 text-sm transition-colors ${
                          location.pathname === item.href
                            ? "bg-surface-1 font-medium text-ink-0"
                            : "text-ink-1 hover:bg-surface-1 hover:text-ink-0"
                        }`}
                      >
                        {item.icon}
                        {item.label}
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-50 border-t border-surface-3 bg-surface-0 lg:hidden">
        <div className="flex items-center justify-around py-2">
          ...
          {/* Mobile "More" button with slide-up panel */}
          <div className="relative" ref={mobileMoreRef}>
            <button
              type="button"
              onClick={() => setMobileMoreOpen((prev) => !prev)}
              className={`flex flex-col items-center gap-1 rounded-lg px-3 py-1.5 ${
                isMobileMoreActive || mobileMoreOpen ? "text-ink-2-interactive" : "text-ink-1"
              }`}
            >
              <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
            ...
            {mobileMoreOpen ? (
              <div className="absolute bottom-full right-0 z-50 mb-2 w-52 rounded-lg border border-surface-3 bg-surface-0 p-1 shadow-card">
