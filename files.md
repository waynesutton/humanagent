...
| `securityUtils.ts` | Security utility functions: input scanning, validation helpers, system prompt builder with supported action types including `generate_audio`, and automatic current date/time injection into agent system prompts (zero DB cost, ~15 tokens) |
...
| `platformApi.ts` | Central typed API contract for frontend pages: Convex function refs (including board project APIs, board `doNow`, workflow steps, `getOutcomeFileUrl`, `getSubtasks`, and settings schedule CRUD/toggle APIs), service catalogs, settings model-catalog refs (`getModelCatalog`, `refreshModelCatalog`), and public HTTP route builders |
